  GNU nano 6.0                                                      simple.c                                                                #include <sys/wait.h>
#include <time.h>
#include <ncurses.h>
#include <dirent.h>
#include <stdio.h>
...
void updateStatusWindow(char* name);
void DestroyALL();
...
//  char* rightListNames[100];
  ...
  //}
       ...
       mvprintw(2 + nStep, 2, "%s", leftListNames[0]);
       nStep++;
    ...
    for(int i = 0; i < number; i++){
      ...
      mvprintw(2+nStep, 2, "%s", returnFilesNames(currentPath, i));
      nStep++;
  }
...
void updateStatusWindow(char* name)
{
   ...
   attron(COLOR_PAIR(3));
   mvprintw(LINES - 2, 2, "%s   %d ", name, strlen(name));
